 ...
 */
export async function detectTsconfigPaths(options: { path?: string, level?: number, cwd?: string } = {}) {
  const paths: string[] = []
  try {
    ...
    level && await Promise.all(
      tsconfigJson.references?.map(async (item) => {
        ...
        if (stats.isDirectory())
          paths.push(...await detectTsconfigPaths({ path: join(item.path, 'tsconfig.json'), level: level - 1, cwd }))
        else if (stats.isFile())
          paths.push(...await detectTsconfigPaths({ path: join(item.path), level: level - 1, cwd }))
      }) ?? [],
    )
  }
  catch (e) {
